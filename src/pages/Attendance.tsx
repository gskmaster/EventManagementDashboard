...
export default function Attendance() {
  ...
  const handleUpdatePerson = async (e: React.FormEvent) => {
    ...
    setIsUpdating(true);
    try {
      const personRef = doc(db, 'persons', editPerson.id);
      const { id, ...updateData } = editPerson;
      await updateDoc(personRef, updateData);
      
      // Update local state
      setPersons(persons.map(p => p.id === editPerson.id ? { ...editPerson } : p));
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <input
                        type="text"
                        required
                        value={editPerson.nik || ''}
                        onChange={(e) => setEditPerson({ ...editPerson, nik: e.target.value })}
                        className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                      />
                    </div>
                    <div>
                      ...
                      <input
                        type="text"
                        required
                        value={editPerson.mobilePhone || ''}
                        onChange={(e) => setEditPerson({ ...editPerson, mobilePhone: e.target.value })}
                        className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                      />
                    </div>
                    <div className="col-span-2">
                    ...
                    <div className="col-span-2">
                      ...
                      <select
                        required
                        value={editPerson.posisi || ''}
                        onChange={(e) => setEditPerson({ ...editPerson, posisi: e.target.value })}
                        className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                      >
                    ...
                    {editPerson.posisi === 'Lainnya' && (
                      <div className="col-span-2">
                        <label className="block text-sm font-medium text-slate-700 mb-1">Sebutkan Posisi</label>
                        <input
                          type="text"
                          required
                          value={editPerson.posisiLainnya || ''}
                          onChange={(e) => setEditPerson({ ...editPerson, posisiLainnya: e.target.value })}
                          className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                        />
                      </div>
                    )}
