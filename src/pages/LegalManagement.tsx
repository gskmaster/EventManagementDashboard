...
import { Search, Shield, Calendar, MapPin, User, ChevronRight, Loader2, FileText } from 'lucide-react';
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <h2 className="text-2xl font-bold text-slate-900">Legal Management</h2>
            </div>
            <p className="text-slate-500">Audit log persetujuan data pribadi peserta (UU PDP No. 27 Tahun 2022).</p>
          </div>
          <button
            onClick={() => navigate('/legal-management/terms')}
            className="flex items-center gap-2 px-4 py-2 border border-slate-300 text-slate-700 text-sm font-medium rounded-lg hover:border-indigo-400 hover:text-indigo-600 transition-colors"
          >
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            {filtered.map(proj => (
              <button
                key={proj.id}
                onClick={() => navigate(`/legal-management/${proj.id}`)}
