...
export default function UsherAttendance() {
  ...
  const fetchProjects = async () => {
    ...
    try {
      ...
      if (role === 'admin' || role === 'event_manager') {
        ...
        // Usher: find assigned projects via ushers collection
        const usherSnap = await getDocs(
          query(
            collection(db, 'ushers'),
  ...
  const fetchPersons = async (project: Project) => {
    try {
      // Fetch persons via institutions (kabupaten-based)
      ...
      // Also fetch persons linked directly by projectId
  ...
  const updateAttendance = async (
    ...
    try {
      ...
      setPersons(prev =>
        prev.map(p => (p.id === personId ? { ...p, attendanceStatus: status } : p))
      );
      const label =
        status === 'present' ? 'Check-in berhasil' : status === 'absent' ? 'Ditandai absen' : 'Status direset';
    ...
    return (
      <div className="p-4 md:p-8 max-w-5xl mx-auto">
        <p className="text-sm text-slate-500 mb-6">
          Pilih proyek untuk mengelola absensi peserta.
        </p>
      ...
      <div className="px-4 md:px-8 py-3 bg-slate-50/80 border-b border-slate-100 space-y-2.5">
        ...
        <div className="flex gap-2 flex-wrap">
          {(
            ...
            ] as { key: AttendanceFilter; label: string; count: number; active: string }[]
          ...
          <div className="text-center py-16">
            <p className="text-slate-500 text-sm">
              {searchTerm || filterStatus !== 'all'
                ...
                : 'Belum ada peserta untuk proyek ini'}
            </p>
          </div>
        ) : (
          <>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {(person.kecamatan || person.desa) && (
                          <p className="text-xs text-slate-400 mt-0.5">
                            {[person.kecamatan, person.desa].filter(Boolean).join(' / ')}
                          </p>
                        )}
                      </div>
                      <span
